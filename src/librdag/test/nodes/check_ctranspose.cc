...
class ReconstructCtransposeNodeTest: public ::testing::TestWithParam<const OGTerminal*> {};

TEST_P(ReconstructCtransposeNodeTest, TerminalTypes)
{
  const OGTerminal * A = GetParam();
  OGExpr * ct = new CTRANSPOSE(A);
  OGExpr * ctctA = new CTRANSPOSE(ct); // use copy else there's two refs to one terminal floating about
  runtree(ctctA);
