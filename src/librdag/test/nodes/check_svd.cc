...
TEST(SVDTests,CheckRealMatrix)
{
  ...
  // answers
  OGTerminal::Ptr U = OGTerminal::Ptr{new OGRealMatrix(new real16[9] {-0.2298476964000714,-0.5247448187602936,-0.8196419411205156,0.8834610176985253,0.2407824921325463,-0.4018960334334317,0.4082482904638627,-0.8164965809277263,0.4082482904638631},3,3,OWNER)};
  OGTerminal::Ptr S = OGTerminal::Ptr{new OGRealDiagonalMatrix(new real16[2] {9.525518091565107,  0.514300580658644},3,2,OWNER)};
  OGTerminal::Ptr VT = OGTerminal::Ptr{new OGRealMatrix(new real16[4]{-0.6196294838293402,-0.7848944532670524,-0.7848944532670524,0.6196294838293402}, 2,2, OWNER)};

  // input
  OGTerminal::Ptr M = OGTerminal::Ptr{new OGRealMatrix(new real16[6]{1,3,5,2,4,6},3,2,OWNER)};
...
TEST(SVDTests,CheckComplexMatrix)
{
  ...
  // answers
  OGTerminal::Ptr U = OGTerminal::Ptr{new OGComplexMatrix(new complex16[9] {{     -0.0228707006002169,      -0.2287070060021659}, {     -0.0522140610036492,      -0.5221406100364927}, {     -0.0815574214070819,      -0.8155742140708198}, {     -0.0879076568710847,      -0.8790765687107978}, {     -0.0239587534423321,      -0.2395875344233279}, {      0.0399901499864153,       0.3999014998641418}, {      0.3147401422050641,       0.2600102104752862}, {     -0.6294802844101258,      -0.5200204209505755}, {      0.3147401422050625,       0.2600102104752883}},3,3,OWNER)};
  OGTerminal::Ptr S = OGTerminal::Ptr{new OGRealDiagonalMatrix(new real16[2] {95.7302720469661779,5.1686568674896343},3,2,OWNER)};
  OGTerminal::Ptr VT = OGTerminal::Ptr{new OGComplexMatrix(new complex16[4]{{-0.6196294838293402,0},{0.7848944532670524,-0.e0},{-0.7848944532670524,0},{-0.6196294838293402,0}}, 2,2, OWNER)};

  // input
  OGTerminal::Ptr M = OGTerminal::Ptr{new OGComplexMatrix(new complex16[6]{{1,10}, {3,30}, {5,50}, {2,20}, {4,40}, {6,60}},3,2,OWNER)};
  OGExpr::Ptr svd = OGExpr::Ptr{new SVD(M)};
  ...
  // Check selecting 0 (U)
  OGExpr::Ptr s0 = OGExpr::Ptr{new SELECTRESULT(svd, OGTerminal::Ptr{new OGIntegerScalar(0)})};
  ExecutionList el0 = ExecutionList{s0};
  for (auto it = el0.begin(); it != el0.end(); ++it)
  {
    d.dispatch(*it);
  ...
  // Check selecting 1 (S)
  OGExpr::Ptr s1 = OGExpr::Ptr{new SELECTRESULT(svd, OGTerminal::Ptr{new OGIntegerScalar(1)})};
  ExecutionList el1 = ExecutionList{s1};
  for (auto it = el1.begin(); it != el1.end(); ++it)
  {
    d.dispatch(*it);
  ...
  // Check selecting 2 (V)
  OGExpr::Ptr s2 = OGExpr::Ptr{new SELECTRESULT(svd, OGTerminal::Ptr{new OGIntegerScalar(2)})};
  ExecutionList el2 = ExecutionList{s2};
  for (auto it = el2.begin(); it != el2.end(); ++it)
  {
    d.dispatch(*it);
