...
INSTANTIATE_NODE_TEST_CASE_P(INVTests,INV,
  ...
  // inv(1) = 1
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealScalar(1.0)}, OGNumeric::Ptr{new OGRealScalar(1.0)}, MATHSEQUAL),
  // inv(-1) = -1
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealScalar(-1.0)}, OGNumeric::Ptr{new OGRealScalar(-1.0)}, MATHSEQUAL),
  // inv(0) = +inf
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealScalar(0.0)}, OGNumeric::Ptr{new OGRealScalar(std::numeric_limits<real16>::infinity())}, MATHSEQUAL),
  // inv(10) = 0.1
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealScalar(10.0)}, OGNumeric::Ptr{new OGRealScalar(0.1)}, MATHSEQUAL),

  // Test matrix context real space

  // inv(1) = 1
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealMatrix(new real16[1]{1},1,1, OWNER)}, OGNumeric::Ptr{new OGRealScalar(1.0)}, MATHSEQUAL),
  // inv(-1) = -1
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealMatrix(new real16[1]{-1},1,1, OWNER)}, OGNumeric::Ptr{new OGRealScalar(-1.0)},
  MATHSEQUAL),
  // inv(0) = +inf
  new CheckUnary<INV>( OGNumeric::Ptr{new OGRealMatrix(new real16[1]{0.e0},1,1, OWNER)}, OGNumeric::Ptr{new OGRealScalar(std::numeric_limits<real16>::infinity())}, MATHSEQUAL),
  // inv(full rank 3x3 system) [condition number ~= 3.7]
  new CheckUnary<INV>(
      OGNumeric::Ptr{new OGRealMatrix(new real16[9]{1,-4,7,2,2,9,3,1,-4},3,3, OWNER)},
      OGNumeric::Ptr{new OGRealMatrix(new real16[9] {0.0918918918918919,0.0486486486486487,0.2702702702702703,-0.1891891891891892,0.1351351351351351,-0.0270270270270270,0.0216216216216216,0.0702702702702703,-0.0540540540540541},3,3, OWNER)},
      MATHSEQUAL),

  // Test matrix context complex space

  // inv(1+0i) = 1
  new CheckUnary<INV>( OGNumeric::Ptr{new OGComplexScalar({1.0,0})}, OGNumeric::Ptr{new OGRealScalar(1.0)}, MATHSEQUAL),
  // inv(0+1i) = -i
  new CheckUnary<INV>( OGNumeric::Ptr{new OGComplexScalar({0,1.0})}, OGNumeric::Ptr{new OGComplexScalar({0,-1.0})}, MATHSEQUAL),
  // inv(0+0i) = inf
  new CheckUnary<INV>( OGNumeric::Ptr{new OGComplexScalar({0,0})}, OGNumeric::Ptr{new OGComplexScalar({std::numeric_limits<real16>::infinity(),0})}, MATHSEQUAL),
  // inv(1+1i) = 0.5-0.5i
  new CheckUnary<INV>( OGNumeric::Ptr{new OGComplexScalar({1.0,1.0})}, OGNumeric::Ptr{new OGComplexScalar({0.5,-0.5})}, MATHSEQUAL),
  // inv(full rank 3x3 system) [condition number ~= 4.9]
  new CheckUnary<INV>(  OGNumeric::Ptr{new OGComplexMatrix(new complex16[9]{{1.,10.}, {-4.,-40.}, {7.,70.}, {2.,20.}, {2.,20.}, {9.,90.}, {3.,30.}, {1.,10.}, {11.,11.}},3,3, OWNER)},
...
TEST(INVTests, NonSquareInput)
{
  ...
  mat = OGTerminal::Ptr{new OGRealMatrix(rdat,3,2,VIEWER)};
  inv = OGExpr::Ptr{new INV(mat)};
  ASSERT_THROW(runtree(inv), rdag_error);

  // real space: try with more cols than rows
  mat = OGTerminal::Ptr{new OGRealMatrix(rdat,2,3,VIEWER)};
  inv = OGExpr::Ptr{new INV(mat)};
  ASSERT_THROW(runtree(inv), rdag_error);

  // complex space: try with more rows than cols
  mat = OGTerminal::Ptr{new OGComplexMatrix(cdat,3,2,VIEWER)};
  inv = OGExpr::Ptr{new INV(mat)};
  ASSERT_THROW(runtree(inv), rdag_error);

  // complex space try with more cols than rows
  mat = OGTerminal::Ptr{new OGComplexMatrix(cdat,2,3,VIEWER)};
  inv = OGExpr::Ptr{new INV(mat)};
  ASSERT_THROW(runtree(inv), rdag_error);
...
TEST(INVTests, WarnOnSingularInput)
{
  ...
  // real space
  mat = OGTerminal::Ptr{new OGRealMatrix(rsingular3x3,3,3,VIEWER)};
  inv = OGExpr::Ptr{new INV(mat)};
  runtree(inv);
  // TODO: assert warn check goes here

  // complex space
  mat = OGTerminal::Ptr{new OGComplexMatrix(csingular3x3,3,3,VIEWER)};
  inv = OGExpr::Ptr{new INV(mat)};
  runtree(inv);
  // TODO: assert warn check goes here
}
