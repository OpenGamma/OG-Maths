...
OGExpr::~OGExpr()
{
  for (std::vector<OGNumeric *>::iterator it = this->_args->begin();
...
void OGExpr::setArgs(std::vector<OGNumeric *> * args)
{
  this->_args = args;
}

size_t OGExpr::getNArgs()
{
  return this->_args->size();
}

void OGExpr::debug_print()
{
  cout << "OGExpr base class" << endl;
}

void OGExpr::accept(Visitor &v)
{
  v.visit(this);
}
...
PLUS::PLUS(OGNumeric* left, OGNumeric* right) :
    OGBinaryExpr(left, right)
{
}

void PLUS::debug_print()
{
...
MINUS::MINUS(OGNumeric* left, OGNumeric* right) :
    OGBinaryExpr(left, right)
{
}

void MINUS::debug_print()
{
