...
public class TestCtransposeMaterialise {

  @DataProvider
  public Object[][] realDataContainer() {
    Object[][] obj = new Object[9][2];

    obj[0][0] = new CTRANSPOSE(new OGRealScalar(1.0));
    obj[0][1] = new OGRealScalar(1.0);

    obj[1][0] = new CTRANSPOSE(new OGRealScalar(-1.0));
    obj[1][1] = new OGRealScalar(-1.0);

    obj[2][0] = new CTRANSPOSE(new OGRealScalar(0.0));
    obj[2][1] = new OGRealScalar(0.0);

    obj[3][0] = new CTRANSPOSE(new OGRealScalar(10.0));
    obj[3][1] = new OGRealScalar(10.0);

    obj[4][0] = new CTRANSPOSE(new OGRealDenseMatrix(1));
    obj[4][1] = new OGRealScalar(1);

    obj[5][0] = new CTRANSPOSE(new OGRealDenseMatrix(-1));
    obj[5][1] = new OGRealScalar(-1);

    obj[6][0] = new CTRANSPOSE(new OGRealDenseMatrix(0));
    obj[6][1] = new OGRealScalar(0);

    obj[7][0] = new CTRANSPOSE(new OGRealDenseMatrix(new double[] { 1, 2, 3 }, 1, 3));
    obj[7][1] = new OGRealDenseMatrix(new double[] { 1, 2, 3 }, 3, 1);

    obj[8][0] = new CTRANSPOSE(new OGRealDenseMatrix(new double[] { 1., -4., 7., -12., 2., 2., 9., 4., 3., 1., 11., 7. }, 4, 3));
    obj[8][1] = new OGRealDenseMatrix(new double[] { 1, 2, 3, -4, 2, 1, 7, 9, 11, -12, 4, 7 }, 3, 4);
    return obj;
  };
  ...
  public Object[][] complexDataContainer() {
    ...
    obj[0][0] = new CTRANSPOSE(new OGComplexScalar(1.0, 0));
    obj[0][1] = new OGComplexDenseMatrix(1.0);

    obj[1][0] = new CTRANSPOSE(new OGComplexScalar(0, 1.0));
    obj[1][1] = new OGComplexScalar(0, -1.0);

    obj[2][0] = new CTRANSPOSE(new OGComplexScalar(0, 0));
    obj[2][1] = new OGComplexScalar(0, 0);

    obj[3][0] = new CTRANSPOSE(new OGComplexScalar(1.0, 1.0));
    obj[3][1] = new OGComplexScalar(1, -1);

    obj[4][0] = new CTRANSPOSE(new OGComplexDenseMatrix(new double[] { 1, 10, -2, 20, 3, -30 }, 1, 3));
    obj[4][1] = new OGComplexDenseMatrix(new double[] { 1, -10, -2, -20, 3, 30 }, 3, 1);

    obj[5][0] = new CTRANSPOSE(new OGComplexDenseMatrix(new double[] { 1, -10, -4, 40, 7, -70, -12, 120, 2, -20, 2, -20, 9, -90, 4, -40, 3, -30, 1, -10, 11, -110, 7, -70 }, 4, 3));
  ...
  public void reconstructionTest(OGExpr input, OGTerminal UNUSED) {
    // twice conjugate transposing the input should equal input.
    OGTerminal expected = Materialisers.toOGTerminal(input);
    OGTerminal answer = Materialisers.toOGTerminal(new CTRANSPOSE(new CTRANSPOSE(input)));
    if (!expected.mathsequals(answer)) {
      throw new MathsException("Terminals not equal. Got: " + answer.toString() + " Expected: " + expected.toString());
    }
  }
